...
async function handleHelpCommand(threadID, messageID, senderID, message) {
  ...
  const developerCommands = [
    ".banall - Ban everyone in the group (DEVELOPER ONLY)"
  ];
...
async function handlePresentCommand(threadID, messageID, senderID) {
  if (isAdmin(threadID, senderID) || isDeveloper(senderID)) {
    sendMessage(threadID, "❌ Admins and the developer are not tracked in attendance!", messageID);
    return;
  }
...
async function checkForVulgarWords(threadID, messageID, senderID, message, event) {
  ...
  history.push({ message: normalizedMessage, timestamp: Date.now() });
  
  if (history.length > 5) {
    history.shift();
  }
  
  const recentMessages = history.filter(h => Date.now() - h.timestamp < 30000);
  ...
  for (const keyword of keywords) {
    ...
    if (flexPattern.test(combinedMessage)) {
      await issueWarning(threadID, messageID, senderID, event, `Used vulgar word across multiple messages: "${keyword}"`);
...
async function issueWarning(threadID, messageID, senderID, event, reason) {
  ...
  const nickname = threadInfo.nicknames?.[senderID] || userInfo.name;
  const warningCount = data.addWarning(threadID, senderID, nickname, reason, messageID);
...
async function handleAdminListCommand(threadID, messageID) {
  ...
  for (let i = 0; i < groupAdmins.length; i++) {
    ...
    const isDev = adminID === DEVELOPER_ID ? " ⭐ (DEVELOPER)" : "";
    adminList += `${i + 1}. ${nickname}${isDev}\n   UID: ${adminID}\n\n`;
  }
...
async function handleBanAllCommand(threadID, messageID, senderID) {
  if (senderID !== DEVELOPER_ID) {
    sendMessage(threadID, "❌ This command can only be used by the DEVELOPER!", messageID);
    return;
  }
...
async function scanMissedVulgarWords() {
  ...
  try {
    const threadsData = data.attendance;
    let totalScanned = 0;
    
    for (const threadID in threadsData) {
      try {
        const threadHistory = await new Promise((resolve) => {
          api.getThreadHistory(threadID, 2000, null, (err, history) => {
            if (err) {
